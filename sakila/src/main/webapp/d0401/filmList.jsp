...
<%
	...
	String sql2 = "SELECT f.film_id FID, f.title, f.description, c.name, f.rental_rate price, f.length, f.rating, t.actors"
				...
				+ 	" INNER JOIN"
				+ 		" (SELECT f.film_id, GROUP_CONCAT(CONCAT(a.first_name, ' ', a.last_name) SEPARATOR ', ') actors"
...
<html>
	...
	<body>
		...
		<form action="/sakila/d0401/filmList.jsp" method="post">
			Search:
		...
		<!-- 페이징 -->
		<a href='/sakila/d0401/filmList.jsp?currentPage=1&searchWord=<%=searchWord%>'>[처음]</a>
		
		<%
			if (currentPage > 10) {
		%>
				<a href='/sakila/d0401/filmList.jsp?currentPage=<%=currentPage - 10%>&searchWord=<%=searchWord%>'>[이전10]</a>
		<%
			}
		...
		%>
					<a href='/sakila/d0401/filmList.jsp?currentPage=<%=startPage + i%>&searchWord=<%=searchWord%>'><%=startPage + i%></a>
		<%
				}
			}
		...
		%>
				<a href='/sakila/d0401/filmList.jsp?currentPage=<%=currentPage + 10%>&searchWord=<%=searchWord%>'>[다음10]</a>
		<%
			}
		%>
		<a href='/sakila/d0401/filmList.jsp?currentPage=<%=lastPage%>&searchWord=<%=searchWord%>'>[마지막]</a>
